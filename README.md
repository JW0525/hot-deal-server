...
뽐뿌 / 클리앙(알뜰구매) / 어미새 / 루리웹(핫딜예판) / 딜바다 / 빠삭 / 이토랜드 핫딜 게시판을 2시간마다
...
hot-deal-server/
  ...
  .github/workflows/crawl.yml  2시간마다 수집 → Cloudflare Pages 게시
  crawler/
    ppomppu.js       뽐뿌 파서
    clien.js         클리앙 파서
    dealbada.js      딜바다 파서
...
배포 수집은 2시간마다(`7 */2 * * *` UTC) 자동으로 다시 실행합니다. 로컬 `npm start`의 스케줄은 매시간입니다.
...
**상시 켜두는 서버를 쓰지 않습니다.** GitHub Actions가 2시간마다 수집해서 결과를
...
```
GitHub Actions (2시간마다 짝수 시각 7분 UTC, 공개 저장소는 무료·무제한)
...
`updatedAt`이 2시간 30분 이내면 정상입니다.

### 비용과 한도

전부 무료이고 카드 등록도 필요 없습니다. 공개 저장소라 Actions 사용 시간이 무제한이고,
Pages 무료 배포는 월 500회이며 2시간 주기는 월 약 360~372회입니다.
...
- 일곱 사이트 모두 커뮤니티 게시판이며 이용약관에 자동 수집·재배포 관련 조항이 있을 수 있습니다.
  개인용 프로토타입 단계를 넘어 여러 사람이 쓰는 서비스로 키울 계획이라면, 각 사이트의
  이용약관을 한 번 확인해보는 걸 권장합니다.
- 배포 크롤링 주기는 2시간에 1번, 사이트당 요청 1회, 사이트 간 1.5초 텀을 두도록 설정되어
  상대 서버에 부담을 주지 않는 수준입니다. 배포 주기는 `.github/workflows/crawl.yml`, 로컬 서버
  주기는 `scheduler.js`에서 각각 관리합니다.
