...
### "이미지 없음" 그림이 썸네일로 딸려 온다 — 반복되는 이미지는 빼낸다 (2026-08-05)

원본 사이트가 이미지 없는 글에 **"이미지 없음" 자리 그림**을 내려준다. 정상 응답(200)에
정상 이미지라 위의 3중 방어로는 하나도 안 걸린다. 앱에서는 회색 네모로 나와 목록이 지저분해진다.

**판별 기준: 바이트까지 똑같은 이미지가 2개 이상의 딜에 쓰이면 그 딜의 사진이 아니다.**
상품 사진은 글마다 다르기 때문이다. `scripts/site.js`의 `dropRepeatedThumbnails()`가
`build` 단계에서 해시로 묶어 `thumbnail = null`로 만들고 파일도 지운다.

2026-08-05 실측(썸네일 337장) — **반복 그룹 5종 17장, 전부 제외 대상이 맞았다**:

| 그룹 | 정체 |
|---|---|
| 어미새 12×11 GIF 8장 | 빈 그림 |
| 뽐뿌 60×50 3장 | 회색 "이미지 없음" |
| 어미새 188×189 2장 | 회색 "이미지 없음" |
| 이토랜드 1,222바이트 2장 | **확장자만 `.jpg`인 SVG** — 앱에서 깨져 보이던 것 |
| 퀘이사존 2장 | 네이버페이 로고 (상품 사진 아님) |

**임계값(`PLACEHOLDER_MIN_DEALS`)을 3으로 올리지 말 것.** 회색 그림이 딱 2번만 나온 경우가 있어 놓친다.

**크기·색상으로 거르려 하지 말 것.** 미니앱 쪽에서 먼저 시도해 봤는데,
색이 거의 없는 이미지 중에 **흰 배경 상품 사진(선풍기·흰 티셔츠)이 섞여 있었다.**
커머스 사진은 흰 배경이 흔해서 진짜 상품 사진을 가려버린다.
(미니앱에는 40px 미만만 거르는 방어가 따로 있다 — `hot-deal/src/App.tsx`의 `MIN_THUMB_PX`)
