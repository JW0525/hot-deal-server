...
| 퀘이사존 수집 | 2026-08-05 | Actions IP가 차단됨. 파서 파일은 남겨뒀으니 VPS 이전 후 되살릴 것 |
...
│   ├── clien.js        클리앙
│   ├── dealbada.js     딜바다 (그누보드 표 구조라 가장 안정적)
│   ├── bbasak.js       빠삭 (그누보드 표 구조)
│   ├── etoland.js      이토랜드 (li 목록 + " | " 구분 메타줄)
│   ├── quasarzone.js   퀘이사존 (현재 SOURCES에서 빠져 있음 — 아래 참고)
...
### 퀘이사존은 뺐다 — GitHub Actions에서 IP로 차단됨 (2026-08-05)
...
- **막히는 곳은 코드가 아니라 IP 문제다.** 후보를 넣기 전에 반드시 `scripts/probe-sites.js`를
  Actions 안에서 돌려 확인할 것. 내 PC에서 200이어도 소용없다.
  2026-08-05 기준 결과: 딜바다·빠삭·이토랜드 통과 / **에펨코리아(430)·아카라이브(403)·
  쿨엔조이(타임아웃)·알구몬(봇차단)·zod(403) 실패.** 이 다섯은 다시 시도하지 말 것.
- **대신 딜바다를 넣었다.** 후보 7곳을 Actions 안에서 직접 찔러본 결과 딜바다만 통과했다
  (쿨엔조이 타임아웃 / 알구몬·에펨코리아 봇차단 / 아카라이브·zod 403). `scripts/probe-sites.js`로 재현 가능.
